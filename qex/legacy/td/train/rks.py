"""Restricted Kohn-Sham DFT solver with trainable exchange-correlation functionals.

This module extends PySCF's restricted Kohn-Sham DFT implementation to support
trainable exchange-correlation (XC) functionals. The solver accepts XC functional
parameters that can be optimized through automatic differentiation.

Important: The main difference from the original PySCF implementation is that the XC functional
parameters are passed through the SCF iterations to parameterize the XC functional
being trained.

Key Features:
    - Parameterized XC functionals that can be trained via gradient descent
    - Modified SCF cycle that propagates functional parameters consistently
    - Automatic differentiation support through the entire SCF procedure
    - Compatible with PySCF's existing DFT infrastructure


See Also:
    pyscf.dft.rks: The base restricted Kohn-Sham implementation
    qex.legacy.td.train.numint: Modified numerical integration routines
"""
...
from qex.legacy.td.train import hf, numint
...
def get_veff(ks, mol=None, dm=None, dm_last=0, vhf_last=0, hermi=1, params=None):
    if mol is None:
        mol = ks.mol
    if dm is None:
        dm = ks.make_rdm1()
    ...
    if hermi == 2:  # because rho = 0
        n, vxc.exc, vxc.vxc = 0, 0.0, 0.0
    else:
        max_memory = ks.max_memory - current_memory()[0]
        n, vxc.exc, vxc.vxc = ni.nr_rks(
            mol,
            ks.grids,
            ks.xc,
            dm,
    ...
    if abs(hyb) < 1e-10 and abs(alpha) < 1e-10:
        ...
        if ground_state:
            vxc.exc -= np.einsum("ij,ji", dm, vk).real * 0.5 * 0.5
    if ground_state:
        vxc.ecoul = np.einsum("ij,ji", dm, vj).real * 0.5
    else:
        vxc.ecoul = None
...
def energy_elec(ks, dm=None, h1e=None, vhf=None, params=None):
    if dm is None:
        dm = ks.make_rdm1()
    if h1e is None:
        h1e = ks.get_hcore()
    if vhf is None or getattr(vhf, "ecoul", None) is None:
        vhf = ks.get_veff(ks.mol, dm, params=params)
