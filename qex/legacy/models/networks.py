...
class LocalMLP(KohnShamNetwork):
    ...
    def build_network(self, grids: jnp.ndarray) -> tuple[Callable, Callable]:
        from qex.legacy.models.classical.classical_models import build_local_mlp
...
class GlobalMLP(KohnShamNetwork):
    ...
    def build_network(self, grids: jnp.ndarray) -> tuple[Callable, Callable]:
        from qex.legacy.models.classical.classical_models import build_global_mlp
...
class BatchedGlobalMLP(KohnShamNetwork):
    ...
    def build_network(self, grids: jnp.ndarray) -> tuple[Callable, Callable]:
        from qex.legacy.models.classical.classical_models import build_batched_global_mlp
...
class LocalQNN(KohnShamNetwork):
    ...
    def build_network(
        ...
        from qex.legacy.models.quantum.quantum_models import build_qnn
...
class GlobalQNN(KohnShamNetwork):
    ...
    def build_network(
        ...
        Returns:
            tuple: (init_fn, apply_fn) pair of network initialization and application functions.
        """
        from qex.legacy.models.quantum.convolutional_models import build_conv_qnn
...
class GlobalQNNReverse(KohnShamNetwork):
    ...
    def build_network(
        ...
        Returns:
            tuple: (init_fn, apply_fn) pair of network initialization and application functions.
        """
        from qex.legacy.models.quantum.convolutional_models import build_conv_qnn_reverse
...
class GlobalQNNClassicalToQuantum(KohnShamNetwork):
    ...
    Model for paper: to test the effects of noise on the training
    ...
    def build_network(
        ...
        Returns:
            tuple: (init_fn, apply_fn) pair of network initialization and application functions.
        """
        from qex.legacy.models.quantum.convolutional_models import build_conv_qnn_classical_to_quantum
...
class GlobalQNNQuantumToClassical(KohnShamNetwork):
    """Convolutional Quantum Neural Network.
    ...
    def build_network(
        ...
        Returns:
            tuple: (init_fn, apply_fn) pair of network initialization and application functions.
        """
        from qex.legacy.models.quantum.convolutional_models import build_conv_qnn_quantum_to_classical

        # Get the input dimension from the grid size
        input_dimension = grids.shape[0]

        # Construct the full convolutional model
        network = build_conv_qnn_quantum_to_classical(
            n_qubits=self.config.get("n_qubits", 6),
            n_var_layers=self.config.get("n_var_layers", 2),
            n_out=1,  # NOTE: not used in this model
            input_dimension=input_dimension,
            normalization=self.config.get("normalization", 1.0),
            ...
            n_shots=self.config.get("n_shots", 0),
            key=self.config.get("key", jax.random.PRNGKey(0)),
            n_features=self.config.get("n_features", 3),
        )
...
class GlobalQiCQNN(KohnShamNetwork):
    ...
    def build_network(
        ...
        Returns:
            tuple: (init_fn, apply_fn) pair of network initialization and application functions.
        """
        from qex.legacy.models.quantum.convolutional_models import build_conv_amplitude_encoding_qnn
...
class GlobalQNNwithMLP(KohnShamNetwork):
    ...
    def build_network(self, grids: jnp.ndarray) -> tuple[Callable, Callable]:
        ...
        Returns:
            tuple: (init_fn, apply_fn) pair of network initialization and application functions.
        """
        from qex.legacy.models.quantum.convolutional_models import build_conv_mlp
