"""This module implements a modified Hartree-Fock SCF solver that accepts parameters
for training exchange-correlation (XC) functionals. The key modifications include:

1. A 'params' argument is passed through the SCF iterations to parameterize the
   XC functional being trained
2. The get_veff() and get_fock() methods accept these parameters to compute the
   XC potential and matrix elements
3. The SCF fixed point iteration and main SCF loop are adapted to handle these
   parameters consistently

This allows for automatic differentiation through the SCF procedure with respect
to the XC functional parameters, enabling gradient-based optimization of the
functional form.
"""
...
from pyscfad import config, lib
from pyscfad import numpy as np
from pyscfad import ops, pytree, util
...
def _scf_fixed_point(dm, mf, s1e, h1e, params):
    vhf = mf.get_veff(mf.mol, dm, params=params)
    fock = mf.get_fock(h1e, s1e, vhf, dm, params=params)
...
def _scf(
    ...
    *,
    ...
    for cycle in range(mf.max_cycle):
        ...
        fock = mf.get_fock(h1e, s1e, vhf, dm, cycle, diis, fock_last=fock_last, params=params)
        mo_energy, mo_coeff = mf.eig(fock, s1e)
        mo_occ = mf.get_occ(mo_energy, mo_coeff)
        dm = mf.make_rdm1(mo_coeff, mo_occ)
        vhf = mf.get_veff(mol, dm, dm_last, vhf, params=params)
...
def kernel(
    ...
    **kwargs,
    ...
    vhf = mf.get_veff(mol, dm, params=params)
    e_tot = mf.energy_tot(dm, h1e, vhf)
    ...
    if config.scf_implicit_diff and (not conv_check or not scf_conv):
        log.warn(
            "\tAn extra scf cycle is going to be run\n"
...
def energy_elec(mf, dm=None, h1e=None, vhf=None, params=None):
    if dm is None:
        dm = mf.make_rdm1()
    if h1e is None:
        h1e = mf.get_hcore()
    ...
    e_coul = np.einsum("ij,ji->", vhf, dm).real * 0.5
...
# # TODO: IGOR for jit scf
# import jax.numpy as jnp
# @wraps(pyscf_hf.make_rdm1)
# def make_rdm1(mo_coeff, mo_occ, **kwargs):
#     """New jit-friendly implementation"""
#     weighted_mo = mo_coeff * jnp.sqrt(mo_occ)[None, :]
#     dm = 2.0 * jnp.dot(weighted_mo, weighted_mo.conj().T)
#     return dm
...
def dip_moment(mol, dm, unit="Debye", verbose=logger.NOTE, **kwargs):
    ...
    if "unit_symbol" in kwargs:
        log.warn(
            'Kwarg "unit_symbol" was deprecated. It was replaced by kwarg '
...
class SCF(pytree.PytreeNode, pyscf_hf.SCF):
    ...
    def scf(self, dm0=None, params=None, **kwargs):
        self.dump_flags()
        self.build(self.mol)
    ...
    def energy_grad(self, dm0=None, mode="rev", params=None):
        ...
        def hf_energy(self, dm0=None, params=None):
            self.reset()
            e_tot = self.kernel(dm0=dm0, params=params)
            return e_tot

        if mode.lower().startswith("rev"):
            jac = jax.grad(hf_energy)(self, dm0=dm0, params=params)
        else:
            if config.scf_implicit_diff:
                msg = """Forward mode differentiation is not available
                         when applying the implicit function differentiation."""
                raise KeyError(msg)
            jac = jax.jacfwd(hf_energy)(self, dm0=dm0, params=params)
        if hasattr(jac, "cell"):
            return jac.cell
        else:
            return jac.mol
    ...
    def get_veff(self, mol=None, dm=None, dm_last=0, vhf_last=0, hermi=1, params=None):
        if mol is None:
            mol = self.mol
        if dm is None:
            dm = self.make_rdm1()
    ...
    def get_occ(self, mo_energy=None, mo_coeff=None):
        ...
        # TODO: IGOR for jit scf
        # return pyscf_hf.SCF.get_occ(self, mo_energy)
...
class RHF(SCF, pyscf_hf.RHF):
    ...
    def check_sanity(self):
        ...
        if mol.nelectron != 1 and mol.spin != 0:
            logger.warn(
                self,
                "Invalid number of electrons %d for RHF method.",
                mol.nelectron,
            )
