...
# Create noise instance for testing
...
def test_error_paulichannel():
    with pytest.raises(ValueError):
        noise = DigitalNoiseInstance(
            DigitalNoiseType.PAULI_CHANNEL,
            error_probability=(0.4, 0.5, 1.1),
        ).kraus
...
def test_error_prob_GeneralizedAmplitudeDamping():
    for p in range(2):
        ...
        with pytest.raises(ValueError):
            noise = DigitalNoiseInstance(
                DigitalNoiseType.GENERALIZED_AMPLITUDE_DAMPING,
                error_probability=probas,
            ).kraus
