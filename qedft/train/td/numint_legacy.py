"""
Legacy implementation of the numerical integration routines from PySCFAD.
"""
...
def eval_mat(
    ...
    if non0tab is None:
        non0tab = numpy.ones(
            ((ngrids + BLKSIZE - 1) // BLKSIZE, mol.nbas),
            dtype=numpy.uint8,
        )
    ...
    if xctype in ["LDA", "HF"]:
        ...
        if spin == 0:
            ...
            try:
                ...
                warnings.warn(
                    "Note the output of libxc.eval_xc cannot be "
    ...
    if xctype == "MGGA":
        ...
        if vlapl is None:
            ...
            aow = _scale_ao(ao2, 0.5 * weight * vlapl, out=None)
            mat += _dot_ao_ao(mol, ao[0], aow, non0tab, shls_slice, ao_loc)
...
def nr_rks(
    ...
    if xctype == "LDA":
        ...
        for ao, mask, weight, coords in ni.block_loop(mol, grids, nao, ao_deriv, max_memory):
            for idm in range(nset):
                ...
                # jax.debug.print("excsum_i: {}", excsum_i)
                # jax.debug.print("vxc: {}", vxc)
    ...
    # Avoid to be able to trace in SCF (originally uncommented)
...
def _rks_mgga_assemble_rho(rho, ao, dm):
    ...
    rho = ops.index_add(rho, ops.index[4], rho[5])
    rho = ops.index_mul(rho, ops.index[4], 2)
    rho = ops.index_mul(rho, ops.index[5], 0.5)
    return rho
...
def _vv10nlc_jvp(
    ...
    exc_jvp = (vxc[0] - exc) / rho[0] * rho_t[0] + vxc[1] / rho[0] * 2.0 * np.einsum(
        "np,np->p",
        rho[1:4],
        rho_t[1:4],
    )
