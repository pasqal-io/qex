...
JIT is supported in this implementation.
"""
...
def _scf_optimality_cond(dm, mf, s1e, h1e, params=None):
...
def _scf(
    ...
    *,
    ...
    for cycle in range(mf.max_cycle):
        ...
        fock = mf.get_fock(h1e, s1e, vhf, dm, cycle, diis, params=params)
        mo_energy, mo_coeff = mf.eig(fock, s1e)
        mo_occ = stop_trace(mf.get_occ)(mo_energy, mo_coeff)
        dm = mf.make_rdm1(mo_coeff, mo_occ)
        ...
        fock = stop_trace(mf.get_fock)(h1e, s1e, vhf, dm, params=params)
...
def kernel(
    ...
    **kwargs,
    ...
    dm, scf_conv, e_tot, mo_energy, mo_coeff, mo_occ = make_implicit_diff(
        _scf,
...
@wraps(pyscf_hf.make_rdm1)
def make_rdm1(mo_coeff, mo_occ, **kwargs):
    """New implementation using einsum"""
    mask = mo_occ > 0
    mocc = mo_coeff[:, mask]  # Changed: Use proper slicing instead of broadcasting
    dm = np.dot(mocc * mo_occ[mask], mocc.conj().T)  # Changed: Use same structure as original
    return dm
...
class SCF(pyscf_hf.SCF):
    ...
    def kernel(self, dm0=None, params=None, **kwargs):
        self.build(self.mol)
