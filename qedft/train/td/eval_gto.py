"""
This file is a modified version of pyscfad.gto.eval_gto.py
to avoid tracing the grid_coords object.
_eval_gto function is the only one that is modified.
"""
...
# Added nondiff_argnums to avoid tracing the grid_coords object
@partial(custom_jvp, nondiff_argnums=(1, 2, 3, 4, 5, 6, 7, 8))
def _eval_gto(
    mol,
    eval_name,
    grid_coords,
...
def _eval_gto_dot_grad_tangent_r0(
    mol,
    mol_t,
    intor,
    shls_slice,
...
def _eval_gto_jvp_r0(mol, mol_t, eval_name, grid_coords, comp, shls_slice, non0tab, ao_loc):
    ...
    tangent_out = _eval_gto_dot_grad_tangent_r0(
        mol,
        mol_t,
        new_eval,
        shls_slice,
    ...
    def _fill_grad():
        ...
        for i in range(shl0, shl1):
            ...
            for j in range(nctr):
                grad[
                    ...,
                    (cs_of[i] + j * nprim) : (cs_of[i] + (j + 1) * nprim),
                    ibas : (ibas + nbas),
                ] += g
                ibas += nbas
            off += nprim * nbas
